...
for (const filter of filterArray) {
    test(`Changing route to ${filter} in URL`, async ({ page }) => {
        ...
        await expect.soft(await angularHomepage.locateTodoBySubstring('Example1')).toBeVisible({visible: filter === 'completed' ? false : true });
        await expect.soft(await angularHomepage.locateTodoBySubstring('Example2')).toBeVisible({visible: filter === 'active' ? false : true });
    });
    ...
    if (filter !== 'completed') {
        test(`Update active todo state when filtering by ${filter}`, async ({ page }) => {
            ...
            await expect.soft(await angularHomepage.locateTodoBySubstring('Example1')).toBeVisible({visible: filter === 'active' ? false : true });
...
test('Check active filter persists on reload', async ({ page }) => {
    ...
    await expect.soft(await angularHomepage.locateTodoBySubstring('Example1')).toBeVisible({visible: true });
    await expect.soft(await angularHomepage.locateTodoBySubstring('Example2')).toBeVisible({visible: false });
});
